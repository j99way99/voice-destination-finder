...
이메일로 회원가입해 자주 가는 곳을 익숙한 이름의 `목적지명`으로 저장해두고,
...
이메일 회원가입 / 로그인  (Firebase Auth)
   ...
   │       { name: '목적지명', mapName, address, lat, lng }
   │
   └─ [음성 검색] 탭 → 두 가지 모드 중 선택
         ├─ 등록된 주소로 찾기 : "우리집" → 저장된 목적지 이름 매칭
